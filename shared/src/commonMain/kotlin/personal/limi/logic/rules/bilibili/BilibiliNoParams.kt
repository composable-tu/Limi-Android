...
fun processBilibiliNoParamsUrl(url: Url): Url {
  if (url.host.lowercase() !in bilibiliNoParamsTargetHost)
    throw UnsupportedURLException("与 bilibiliNoParamsTargetHost 链接不匹配")
  val finalUrl =
    URLBuilder(url)
      .apply {
      ...
      .build()
  if (finalUrl.host.lowercase() !in bilibiliNoParamsAllowedHost)
    throw UnsupportedURLException("重定向链接与 bilibili.com 链接不匹配")
  return finalUrl
}
