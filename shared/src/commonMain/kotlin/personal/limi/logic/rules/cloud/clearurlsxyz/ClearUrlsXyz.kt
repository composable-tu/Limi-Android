...
@Serializable
data class ClearUrlsCatalog(
    val providers: Map<String, ClearUrlsProvider> = emptyMap(),
)
...
private fun providerMatches(provider: ClearUrlsProvider, urlString: String): Boolean =
    provider.compiledUrlPattern?.containsMatchIn(urlString) == true && provider.compiledExceptions.none {
...
private fun removeFields(urlString: String, rules: List<Regex>): String {
    ...
    for (entry in url.parameters.entries()) {
        if (rules.any { it.matches(entry.key) }) queryChanged =
            true else builder.parameters.appendAll(entry.key, entry.value)
    }

    var fragmentChanged = false
    val encodedFragment = url.encodedFragment
    if (encodedFragment.isNotEmpty()) {
        val kept = encodedFragment.split('&').filter { part ->
