...
suspend fun processUrl(urlString: String): String {
  ...
  val ruleConfig =
    RuleConfig(
      ...
      firefoxQueryStripping =
        DataStorePreferences.getBoolean(RuleIds.FIREFOX_QUERY_STRIPPING, false),
  ...
  finalUrl =
    when {
      ruleConfig.bilibili && finalUrl.host.lowercase() in bilibiliRedirectTargetHost ->
        processBilibiliRedirectUrl(finalUrl)

      ruleConfig.bilibili && finalUrl.host.lowercase() in bilibiliNoParamsTargetHost ->
        processBilibiliNoParamsUrl(finalUrl)
  ...
  if (ruleConfig.firefoxQueryStripping) {
    val stripList =
      DataStorePreferences.getStringSet(FirefoxRuleKeys.QUERY_STRIPPING_STRIP_LIST, emptySet())
  ...
  if (ruleConfig.braveQueryFilter) {
    val rules =
      decodeQueryFilterRules(DataStorePreferences.getString(QueryFilterRuleKeys.RULES, ""))
