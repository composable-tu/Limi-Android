...
interface LimiHistoryDao {
  @Insert suspend fun insert(item: LimiHistoryEntity)

  @Query("SELECT count(*) FROM LimiHistoryEntity") suspend fun count(): Int

  @Query("SELECT * FROM LimiHistoryEntity") fun getAllAsFlow(): Flow<List<LimiHistoryEntity>>
  ...
  @Delete suspend fun delete(item: LimiHistoryEntity)
}
